                    ...
                    {orderTypes.map((v, i) => {
                        if (v.isActive)
                            return (
                                <View
                                    key={i}
                                    style={{ ...AdvisorStyles.setFlex, ...AdvisorStyles.viewProfile, paddingTop: 20, borderTopColor: 'rgba(0, 0, 0, 0.5)', borderTopWidth: 0.5, justifyContent: 'space-between', paddingBottom: 10 }}
                                >
                                    <View style={AdvisorStyles.orderView}>
                                    ...
                                    <Button
                                        title={`$ ${roundToTwo(v.price)} ${v.orderTypeName.toLowerCase().indexOf('live') !== -1 ? '/ min' : ''}`} containerStyle={{ width: 105 }}
                                        buttonStyle={{ backgroundColor: appColor }}
                                        onPress={() => validateOrder(v.orderTypeName)}
                                    />
                                </View>
                            )
                    })}
